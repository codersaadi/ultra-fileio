  ...
  implements IFileRepository
{
  ...
  async createFile(data: FileInsert): Promise<FileRecord> {
    return this.wrapWithHooks(
      ...
      async () => {
        try {
          const [file] = await this.db.insert(this.filesTable).values(data).returning();
          if (!file) {
  ...
  async deleteFile(id: string): Promise<void> {
    return this.wrapWithHooks(
      ...
      async () => {
        try {
          await this.db.delete(this.filesTable).where(this.drizzleFns.eq(this.filesTable.id, id));
        } catch (error) {
          throw new StorageError({
            code: 'INTERNAL_SERVER_ERROR',
    ...
    return this.wrapWithHooks(
      ...
      async () => {
        try {
          ...
          if (search) {
            conditions.push(this.drizzleFns.like(this.filesTable.originalFilename, `%${search}%`));
          }

          if (uploaderId) {
            conditions.push(this.drizzleFns.eq(this.filesTable.uploadedBy, uploaderId));
          }

          if (startDate) {
            conditions.push(this.drizzleFns.gte(this.filesTable.createdAt, startDate));
          }

          if (endDate) {
            conditions.push(this.drizzleFns.lte(this.filesTable.createdAt, endDate));
          }
          ...
          if (this.users) {
            data = await this.db
              ...
              .from(this.filesTable)
              .leftJoin(this.users, this.drizzleFns.eq(this.filesTable.uploadedBy, this.users.id))
              .where(whereClause)
              .orderBy(orderDir === 'desc' ? this.drizzleFns.desc(this.filesTable[orderBy]) : this.filesTable[orderBy])
              .limit(limit)
              .offset(offset);
          } else {
    ...
    return this.wrapWithHooks(
      ...
      async () => {
        try {
          ...
          const result = await this.db.delete(this.filesTable.files).where(this.drizzleFns.inArray(this.filesTable.id, ids));
    ...
    try {
      ...
      const statsResult = await this.db
        .select({
          ...
          totalSize: this.drizzleFns.sql<number>`COALESCE(SUM(${this.filesTable.fileSize}), 0)`,
          averageFileSize: this.drizzleFns.sql<number>`COALESCE(AVG(${this.filesTable.fileSize}), 0)`,
        })
        .from(this.filesTable);
  ...
  async count(options: FileQueryOptions = {}): Promise<number> {
    try {
      ...
      const whereClause = conditions.length > 0 ? this.drizzleFns.and(...conditions) : undefined;
