...
import { R2StorageService, createR2StorageService, StorageError } from './file.module';
import { IFileRepository, FileRecord, FileInsert } from './repositories/file.repository.interface';
...
export class FlexibleFileService {
   ...
   */
  constructor(repository: IFileRepository, config:FileServiceConfig = getDefaultFileServiceConfig()){
    ...
    try {
      ...
      if (request.file.size > this.config.maxFileSize) {
        throw new StorageError({
          ...
          metadata: { maxSize: this.config.maxFileSize, actualSize: request.file.size },
        });
      }
   ...
   */
  async deleteFile(fileId: string, userId?: string): Promise<{ file: FileRecord; r2Deleted: boolean }> {
...
export const getDefaultFileServiceConfig = (): FileServiceConfig => {
  const requiredR2Vars = ['S3_ACCOUNT_ID', 'S3_ACCESS_KEY_ID', 'S3_SECRET_ACCESS_KEY', 'S3_BUCKET_NAME'];
