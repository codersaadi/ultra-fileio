...
export function usePresignedUpload(
  ...
  const config = useMemo(() => ({
    getPresignedUrlEndpoint: options?.getPresignedUrlEndpoint || '/api/fileuploads/upload-url',
    saveFileRecordEndpoint: options?.saveFileRecordEndpoint || '/api/fileuploads/save-file',
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          xhr.upload.addEventListener('progress', (e) => {
            if (e.lengthComputable) {
              ...
              const uploadPercentage = Math.round(((e.loaded / e.total) * 70) + 20);
              const progressData = {
                loaded: e.loaded,
                total: e.total,
          ...
          xhr.addEventListener('load', () => {
            if (xhr.status >= 200 && xhr.status < 300) {
              setProgress({ loaded: file.size, total: file.size, percentage: 90 });
              resolve();
            } else {
              const error = new Error(`Upload to storage failed: ${xhr.statusText}`);
              setError(error);
