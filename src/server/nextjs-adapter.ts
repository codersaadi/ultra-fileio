import {  FileHandlerConfig, createFileHandler } from './file-handler';
...
export const createNextFileHandler = (config: FileHandlerConfig) => {
  if (!config.fileService || !config.fileRepository) {
    ...
    return Response.json(
      {
        error: 'File service not configured. Please check your .env file and ensure R2/S3 credentials are properly set.',
        details: 'Missing required environment variables: S3_ACCOUNT_ID, S3_ACCESS_KEY_ID, S3_SECRET_ACCESS_KEY, S3_BUCKET_NAME'
      },
      { status: 500 }
    )
  }
