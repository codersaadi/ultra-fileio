...
export class FileHandler {
  ...
  async handle(request: Request): Promise<FileHandlerResponse> {
    ...
    try {
      ...
      if (method === 'POST' && pathSegments[0] === 'files' && !pathSegments[1]) {
      ...
      if (method === 'GET' && pathSegments[0] === 'files' && pathSegments[1] && !pathSegments[2]) {
        return await this.handleGetFile(request, pathSegments[1]);
      }

      // DELETE /files/:id - Delete file
      if (method === 'DELETE' && pathSegments[0] === 'files' && pathSegments[1] && !pathSegments[2]) {
        return await this.handleDeleteFile(request, pathSegments[1]);
      }

      // GET /files/:id/download - Download file
      if (method === 'GET' && pathSegments[0] === 'files' && pathSegments[1] && pathSegments[2] === 'download') {
   ...
   * POST /upload-url - Generate presigned upload URL
   */
  private async handlePresignedUrl(request: Request): Promise<FileHandlerResponse> {
  ...
  private async handleSaveFile(request: Request): Promise<FileHandlerResponse> {
    ...
    if (!r2Key || !originalFilename || !fileSize || !publicUrl) {
      return {
        status: 400,
        body: { error: 'Missing required fields: r2Key, originalFilename, fileSize, publicUrl' },
      };
    }
   ...
   * POST /files - Direct file upload (multipart form)
   */
  private async handleDirectUpload(request: Request): Promise<FileHandlerResponse> {
    const userId = await this.config.getUserId(request);
   ...
   * GET /files - List all files for user
   */
  private async handleListFiles(request: Request): Promise<FileHandlerResponse> {
    const userId = await this.config.getUserId(request);
   ...
   * GET /files/:id - Get specific file
   */
  private async handleGetFile(request: Request, fileId: string): Promise<FileHandlerResponse> {
   ...
   * DELETE /files/:id - Delete file
   */
  private async handleDeleteFile(request: Request, fileId: string): Promise<FileHandlerResponse> {
    const userId = await this.config.getUserId(request);
   ...
   * GET /files/:id/download - Download file
   */
  private async handleDownloadFile(request: Request, fileId: string): Promise<FileHandlerResponse> {
    const file = await this.config.fileRepository.getFileById(fileId);
    ...
    const signedUrl = await this.config.fileService?.r2Service.generateSignedUrl(file.r2Key, 3600);
